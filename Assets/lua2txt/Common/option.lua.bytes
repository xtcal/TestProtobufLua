...
local copy2=function(self, copied)
    ...
    -- copy it
    for k, v in pairs(wrap(copied)) do
        self[k] = v
    end

end
-- clear the table
local copy2=function(self)
...
function option.init(menu)
        ...
        if prefix ~= 0 then
            ...
            if prefix == 1 and opt[3] == "kv" then
                ...
                if idx == nil or (arg:startswith("-") and not arg:find("%s")) then 
                    ...
                    -- invalid option
                    return false, "invalid option: " .. option.ifelse(idx, arg, key)
                end
    ...
    for _, o in ipairs(wrap(option.taskmenu().options)) do
        ...
        if prefix ~= 0 then
            ...
            if prefix == 1 and opt[3] == "kv" then
                ...
                if idx == nil or (arg:startswith("-") and not arg:find("%s")) then 
                    ...
                    -- failed
                    return nil, "invalid option: " .. option.ifelse(idx, arg, key)
                end
...
function option.show_menu(task)
    ...
    if taskmenu.usage then
        print("")
        print("${bright}Usage: $${default cyan}" .. taskmenu.usage)
    end
...
function option.show_main()
    ...
    if main.usage then
        print("")
        print("${bright}Usage: $${default cyan}" .. main.usage)
    end
    ...
    if main.tasks then
        ...
        for _, categoryinfo in ipairs(categories_sorted) do
            ...
            -- print category name
            print("")
            print(string.format("${bright}%s%ss: ", string.sub(categoryname, 1, 1):upper(), string.sub(categoryname, 2)))
...
function option.show_options(options)
    ...
    for _, opt in ipairs(options) do
        ...
        if shortname then
            ...
            if mode == "kv" then
                option_info = option_info .. " " .. option.ifelse(name, name:upper(), "XXX")
            end
        end
        ...
        if name then
            ...
            elseif mode == "vs" then
                ...
                option_info = option_info .. option.ifelse(shortname, ", --", "        --") .. name
            end
            if mode == "kv" then
                option_info = option_info .. "=" .. option.ifelse(type(default) == "boolean", "[y|n]", name:upper())
            end
        ...
        if default then
            ...
            if type(default) == "boolean" then
                defaultval = option.ifelse(default, "y", "n")
            end
            ...
            origin_width = option.ifelse(origin_width + #defaultval > console_width, option._get_linelen(option_info), origin_width + (#(tostring(default))))
        ...
        if values then
            ...
            for _, value in ipairs(wrap(values)) do
